...
//        processRequest(request, response);
        ...
        if(content!=null){
            dao.insertComment(username, pid, content, 0);
        }else{
            dao.insertComment(acc.getUsername(), pid, reply, Integer.parseInt(request.getParameter("cid")));
        }
        request.setAttribute("account", acc);
        request.setAttribute("product", dao.getProductById(pid));
        request.setAttribute("comment", dao.getCommentByProductId(pid));
